...
def dcg(ranking):
    """ menghitung search effectiveness metric score dengan 
        ...
        Float
          score DCG
    """
    # Reference: https://web.stanford.edu/class/cs276/handouts/EvaluationNew-handout-1-per.pdf (Slide 15)
    score = 0

    # Rumus dimulai dari 1, lakukan penyesuaian untuk index dan gain
    for i in range(1, len(ranking) + 1):
        gain = 2 ** ranking[i - 1] - 1
...
def prec(ranking, k):
    """ menghitung search effectiveness metric score dengan 
        ...
        Float
          score Prec@K
    """
    # Reference: https://datascience.stackexchange.com/questions/92247/precisionk-and-recallk
    # Mengambil jumlah data yang relevan (bernilai 1) di top-k, kemudian bagi dengan k
...
def ap(ranking):
    ...
    R = sum(ranking) # number of relevant documents
    if R == 0:
        return 0
    total = 0
    for r in range(len(ranking)):
        precision_at_r = prec(ranking, r + 1) * ranking[r]
        total += precision_at_r
    return total/R
